...
TEST_CASE( "Numbers are parsed", "[expr]" ) {
    ...
    auto parse_string = [&driver](std::string&& str) -> box<SelectStatement> {
    ...
    SECTION( "integer" ) {
		auto result = parse_string("select 192");
...
//		auto &resultStmt = driver.getResult();
//		REQUIRE(std::holds_alternative<box<SelectStatement>>(resultStmt));
//		auto &select = std::get<box<SelectStatement>>(resultStmt);
//		REQUIRE(std::holds_alternative<>(resultStmt))
